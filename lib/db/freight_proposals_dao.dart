...
final String table = "proposals_freight";
final String hashColumn = "hash";

class FreightProposalsDAO {
  final DatabaseProvider _databaseProvider;

  FreightProposalsDAO(this._databaseProvider);
  ...
  // Receive a list of proposals and store it in the DB and return a list of the results of the insertions
  Future<List<int>> insertAll(List<FreightDetails> rows) async {
    List<int> results = new List<int>();
    for (FreightDetails row in rows) {
      results.add(await insert(row));
    }
    return results;
  }
  ...
  Future<List<FreightDetails>> queryAllRows() async {
    Database db = await _databaseProvider.database;
    List<FreightDetails> list = new List<FreightDetails>();
  ...
  Future<List<int>> deleteAll() async {
    List<FreightDetails> freightsDetails = await this.queryAllRows();
    List<int> resultDeleteAll = new List<int>();
    
    for(FreightDetails freightDetail in freightsDetails){
      resultDeleteAll.add(await this.delete(freightDetail.hash));
    }
